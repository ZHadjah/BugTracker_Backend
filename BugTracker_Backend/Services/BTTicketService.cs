...
namespace BugTracker_Backend.Services
{
    public class BTTicketService : IBTTicketService
    {
        ...
        public async Task<List<Ticket>> GetProjectTicketsByPriorityAsync(string priorityName, int companyId, int projectId)
        {
            ...
            try
            {
                tickets = (await GetAllTicketsByStatusAsync(companyId, priorityName)).Where(t => t.ProjectId == projectId).ToList();
                return tickets;
            }
            catch (Exception)
            {
        ...
        public async Task<List<Ticket>> GetProjectTicketsByTypeAsync(string typeName, int companyId, int projectId)
        {
            ...
            try
            {
                tickets = (await GetAllTicketsByStatusAsync(companyId, typeName)).Where(t => t.ProjectId == projectId).ToList();
                return tickets;
        ...
        {            
            try
            {
                ...
                List<Ticket> tickets = role switch
                {
                    ...
                    "Developer" => tickets = (await GetAllTicketsByCompanyAsync(companyId)).Where(t => t.DeveloperUserId == userId).ToList(),
                
                    "Submitter" => tickets = (await GetAllTicketsByCompanyAsync(companyId)).Where(t => t.OwnerUserId == userId).ToList(),
                
                    "ProjectManager" => tickets = await GetTicketsByUserIdAsync(userId, companyId)
                };
