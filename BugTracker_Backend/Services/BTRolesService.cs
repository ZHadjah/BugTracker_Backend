...
namespace BugTracker_Backend.Services
{
    public class BTRolesService : IBTRolesService
    {
        ...
        public async Task<List<BTUser>> GetUsersInRoleAsync(string roleName, int companyId)
        {
            ...
            List<BTUser> result = users.Where(c => c.CompanyId == companyId).ToList(); 
            return result;     
        }
        ...
        {
            //Get all the users                                                      | Get all the user's Guid's (individual id's
            List<string> userIds = (await _userManager.GetUsersInRoleAsync(roleName)).Select(u => u.Id).ToList();

            //Check users inside the database and put all the users without a role inside a list
            List<BTUser> roleUsers = _context.Users.Where(x => !userIds.Contains(x.Id)).ToList();

            List<BTUser> result = roleUsers.Where(c => c.CompanyId == companyId).ToList();
